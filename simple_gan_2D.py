...
def two_gaussians_distribution_sampler(mu1, sigma1, mu2, sigma2):
    def gaussians(n):
        g1 = torch.Tensor(np.random.normal(mu1, sigma1, (1, n//2)))
        g2 = torch.Tensor(np.random.normal(mu2, sigma2, (1, n//2)))
        sample = torch.cat((g1, g2), 1)
        return sample[:, torch.randperm(n)]
    return gaussians
...
def make_histogram(data_real, data_fake, i, n_bins=100):
    ...
    axes.set_xlim(-5, 8)
    axes.set_ylim(0, 1)
    fig.savefig("iteration_{}.png".format(i))
    plt.close(fig)
...
def main():
    ...
    data_sampler = two_gaussians_distribution_sampler(-2, 1, 4, 0.5)
